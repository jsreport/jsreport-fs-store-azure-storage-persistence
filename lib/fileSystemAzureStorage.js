...
module.exports = ({ accountName, accountKey }) => {
  ...
  return {
    ...
    rename: async (p, pp) => {
      const blobsToRename = await blobService.listBlobsSegmentedWithPrefixAsync(container, p.replace(/^\//, ''), null)
      return Promise.all(blobsToRename.entries.map(async (e) => {
        const newName = e.name.replace(p.replace(/^\//, ''), pp.replace(/^\//, ''))
    ...
    remove: async (p) => {
      const blobsToRemove = await blobService.listBlobsSegmentedWithPrefixAsync(container, p.replace(/^\//, ''), null)
