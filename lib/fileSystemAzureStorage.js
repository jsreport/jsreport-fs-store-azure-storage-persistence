...
module.exports = ({ accountName, accountKey, container = 'jsreport', lock = {} }) => {
  ...
  return {
    ...
    writeFile: (p, c) => {
      return blobService.createBlockBlobFromTextAsync(container, p, c)
    },
    appendFile: async function (p, c) {
      ...
      return blobService.createBlockBlobFromTextAsync(container, p, Buffer.concat([existingBuffer, Buffer.from(c)]))
    },
    rename: async (p, pp) => {
    ...
    copyFile: (p, pp) => blobService.startCopyBlobAsync(blobService.getUrl(container, []), container, pp),
    path: {
