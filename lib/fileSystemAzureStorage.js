...
module.exports = ({ accountName, accountKey, container = 'jsreport', lock = {} }) => {
  ...
  return {
    ...
    lock: () => lock ? retry(() => blobService.acquireLeaseAsync(container, null, lock), lock.retry) : null,
    releaseLock: (l) => lock ? blobService.releaseLeaseAsync(container, null, l.id) : null
  }
}
